...
def test_detect_generate_and_install_claude_skills_for_representative_stack(
    ...
    installed_paths = {item.path.relative_to(install_root).as_posix() for item in installed}
    ...
    assert all(_is_relative_to(item.path.resolve(), install_root.resolve()) for item in installed)
