...
def test_supported_agents_match_documented_targets(tmp_path: Path) -> None:
    ...
    for agent, relative_target in EXPECTED_TARGETS.items():
        ...
        assert adapter.target_directory(project) == (project / relative_target).resolve()


def test_invalid_agent_lists_supported_names() -> None:
